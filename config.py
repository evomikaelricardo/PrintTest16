...
# Replace this with your own base64-encoded logo
ICON_BASE64 = "YOUR_BASE64_ENCODED_LOGO_HERE"
...
def show_custom_message(parent, title, message, message_type="info", buttons=None):
    """
    Show a custom message dialog with the application logo.
    
    Args:
        parent: Parent window
        title: Dialog title
        message: Message text
        message_type: "info", "warning", "error", or "question"
        buttons: List of button texts (default: ["OK"])
    
    Returns:
        Selected button text or None if closed
    """
    if buttons is None:
        buttons = ["OK"]
    ...
    # Main frame
    main_frame = tk.Frame(dialog, bg=CARD_COLOR)
    ...
    icon_image = get_icon_image()
    if icon_image:
        icon_label = tk.Label(header_frame, image=icon_image, bg=CARD_COLOR)
    ...
    # Button frame
    button_frame = tk.Frame(main_frame, bg=CARD_COLOR)
    ...
    for i, button_text in enumerate(buttons):
        if i == 0:  # First button is primary
            btn = tk.Button(
                button_frame,
                text=button_text,
        ...
        else:  # Other buttons are secondary
            btn = tk.Button(
                button_frame,
                text=button_text,
                command=lambda txt=button_text: on_button_click(txt),
